...
const EDITORIAL_CONTENT: Record<string, {
  ...
  'midnight-drive': {
    ...
    description: 'There\'s something magical about driving through empty streets as the city lights blur past. The right playlist transforms a simple commute into a cinematic experience. This collection captures the essence of those late-night journeys.',
    heroColor: '#1a1a2e',
    sections: [
  ...
  'rainy-tamil-nights': {
    ...
    description: 'The pitter-patter of rain against the window, the earthy scent of wet soil, and the soulful strains of Tamil melodies create an atmosphere of profound introspection.',
    heroColor: '#1a2332',
    sections: [
      { id: 'essay', label: 'The Monsoon Mood', type: 'text', content: 'Rain in Tamil Nadu is not just weather — it is an emotion, a memory, a feeling that runs deep in the cultural consciousness. These songs capture that essence, weaving tales of love, longing, and the beauty of rain-soaked evenings.' },
      { id: 'tracks', label: 'Rainy Day Selections', type: 'songs' },
      { id: 'reflection', label: 'A Note', type: 'text', content: 'Each melody carries the weight of monsoon memories — first loves, quiet reflections, and the simple joy of watching rain fall through a window.' },
    ],
  },
  'synthwave-dreams': {
    title: 'Synthwave Dreams',
    subtitle: 'Retro futuristic electronic soundscapes',
    description: 'A journey through neon-lit streets and retro-futuristic soundscapes. Where analog warmth meets digital precision.',
    heroColor: '#2d1b3d',
    sections: [
      { id: 'essay', label: 'The Retro Future', type: 'text', content: 'Synthwave is a genre built on nostalgia for a future that never was. It evokes images of neon-drenched cityscapes, vintage arcades, and the romanticism of early computing. The pulsating basslines and shimmering arpeggios create a sound that is both familiar and otherworldly.' },
      { id: 'tracks', label: 'The Collection', type: 'songs' },
      { id: 'culture', label: 'Cultural Impact', type: 'text', content: 'From Stranger Things to Drive, synthwave has permeated modern media, becoming the sonic signature of a generation that romanticizes the 80s aesthetic.' },
    ],
  },
}
  ...
  return (
    <div>
      ...
      >
        ...
        <Link
          to="/"
          className="absolute top-6 left-5 lg:left-8 z-20 w-10 h-10 rounded-full flex items-center justify-center transition-all hover:scale-105"
        ...
        {editorial.sections.map((section) => (
          <section key={section.id} id={section.id} className="mb-16">
            {section.type === 'text' && section.content && (
              <FadeInView>
                <h2 className="text-[24px] lg:text-[32px] font-bold tracking-tight mb-4">{section.label}</h2>
                <p className="text-[15px] lg:text-[16px] leading-relaxed" style={{ color: 'var(--am-text-2)' }}>
                  {section.content}
                </p>
              </FadeInView>
            )}
            ...
            {section.type === 'songs' && allSongs.length > 0 && (
              <div>
                <h2 className="text-[24px] lg:text-[32px] font-bold tracking-tight mb-6">{section.label}</h2>
      ...
      {/* Dynamic Island TOC */}
      <DynamicIslandTOC
        items={editorial.sections.map(s => ({ id: s.id, label: s.label }))}
      />
    </div>
  )
}
